...
async function main() {
  ...
  const { Client, ClientAddress, ClientDevice, Partner, PartnerService, RepairOrderModel, Review } = await import("../src/models");
  ...
  try {
    ...
    if (process.argv[2] === "migrate") {
      ...
      const pending = ["20260909_contributions.sql", "20260916_database_persistence.sql", "20260917_normalize_addresses.sql", "20260918_der.sql", "20260919_device_issue.sql", "20260920_partner_services.sql", "20260921_support_chat.sql"]
      ...
      await sequelize.transaction({ isolationLevel: Transaction.ISOLATION_LEVELS.REPEATABLE_READ }, async (transaction) => {
        for (const table of ["clients", "partner", "client_addresses", "client_devices", "devices", "workflow_records", "repair_orders", "reviews", "partner_services", "support_messages"]) {
    ...
    const models: ModelStatic<Model>[] = [Client, ClientAddress, ClientDevice, Partner, PartnerService, RepairOrderModel, Review, Workflow];
    ...
    const [rows] = await sequelize.query(`
      ...
      WHERE c.relnamespace = 'public'::regnamespace
        AND c.relname IN ('clients','client_addresses','devices','partner','partner_services','repair_orders','reviews','workflow_records','support_messages')
      ORDER BY c.relname
    `);
    console.table(rows);
    if (rows.length !== 9 || rows.some((row) => {
